const cursosRepository = require("../repository/cursos-repository");

// Função para retornar todos os cursos
const retornaTodosCursos = async (req, res) => {
	try {
		const cursos = await cursosRepository.obterTodosCursos();
...
const criaCurso = async (req, res) => {
	...
	try {
		const curso = await cursosRepository.criarCurso(formData);
		res.sendStatus(200);
	} catch (error) {
...
const atualizaCurso = async (req, res) => {
	const formData = req.body.formData;
	try {
		const sucesso = await cursosRepository.atualizarCurso(
			formData.id,
			formData,
		);
...
const deletaCurso = async (req, res) => {
	try {
		...
		const sucesso = await cursosRepository.deletarCurso(id);
